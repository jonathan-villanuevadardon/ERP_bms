...
@section('title', 'Editar Permiso Pendiente')

@section('content')
<div class="mb-4">
    <a href="{{ route('permisos.pendientes') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i> Volver</a>
    <h1 class="h3 mt-2">Editar permiso de #{{ $pendiente->clave }}</h1>
</div>

<div class="card shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ route('permisos.actualizar_pendiente', $pendiente) }}">
            @csrf @method('PUT')
            <div class="row">
        ...
        <form id="formEliminar" method="POST" action="{{ route('permisos.eliminar_pendiente', $pendiente) }}" class="d-none">
            @csrf @method('DELETE')
        </form>
    </div>
</div>
...
<script>
function eliminar() {
    if (confirm('¿Eliminar este permiso pendiente?')) {
